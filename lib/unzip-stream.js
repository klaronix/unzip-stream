...
UnzipStream.prototype.processDataChunk = function (chunk) {
    ...
    switch (this.state) {
        ...
        case states.CENTRAL_DIRECTORY_FILE_HEADER_SUFFIX:
            ...
            var isDirectory = this.parsedEntity.uncompressedSize === 0 && /[\/\\]$/.test(path);
            var entry = {
                // protect against malicious zip files which want to extract to parent dirs
                path: path.replace(/^([/\\]*[.]+[/\\]+)*[/\\]*/, ""),
                type: isDirectory ? 'Directory' : 'File',
                isDirectory: isDirectory,
                size: this.parsedEntity.uncompressedSize,
                compressedSize: this.parsedEntity.compressedSize
            };

            try {
                let d = new Date();
                d.setFullYear(1980+(this.parsedEntity.lastModifiedDate>>9));
                d.setMonth(((this.parsedEntity.lastModifiedDate>>5) & 0x0F) - 1);
                d.setDate(this.parsedEntity.lastModifiedDate & 0x1F);

                d.setHours(this.parsedEntity.lastModifiedTime>>11);
                d.setMinutes((this.parsedEntity.lastModifiedTime>>5) & 0x3F);
                d.setSeconds((this.parsedEntity.lastModifiedTime & 0x1F) * 2);
                d.setMilliseconds(0);

                entry.date = d;
            } catch(e) {}
        
            this.emit("central-directory-entry", entry);
